...
export const extractInvoiceData = (
  ...
  console.log(
    `Đã xử lý ${processedCount} dòng, có ${
      Object.keys(result).length
    } số hóa đơn duy nhất và ${allInvoices.length} hóa đơn tổng cộng`
  );
...
export const findDuplicateInvoices = (
  ...
  Object.entries(occurrences).forEach(([_signature, items]) => {
    ...
    if (
      items.length > 1 &&
      items.every((item) => isNumber(item.invoiceOriginal))
    ) {
      ...
      console.log(
        `File ${isFile1 ? "1" : "2"}: Tìm thấy ${
          items.length
        } bản ghi trùng lặp hoàn toàn cho hóa đơn ${
          items[0].invoiceOriginal
        } - ${items[0].seller}`
      );
    }
  });
  ...
  console.log(
    ...
    } hóa đơn trùng lặp hoàn toàn trong file ${isFile1 ? "1" : "2"}`
  );
...
export const findCrossFileDuplicates = (
  ...
  commonKeys.forEach((key) => {
    ...
    file1Items.forEach((item1) => {
      file2Items.forEach((item2) => {
        const netPrice1 =
          parseFloat(String(item1.netPrice || "0").replace(/[^0-9.-]/g, "")) ||
          0;
        const netPrice2 =
          parseFloat(String(item2.netPrice || "0").replace(/[^0-9.-]/g, "")) ||
          0;
        ...
        if (isDoublePrice) {
          ...
          console.log(
            `Tìm thấy hóa đơn trùng lặp với giá gấp đôi: ${item1.invoiceOriginal} - ${item1.seller}`
          );
          console.log(
            `File 1 netPrice: ${netPrice1}, File 2 netPrice: ${netPrice2}`
          );
        }
      });
    });
  ...
  console.log(
    `Tổng cộng: ${duplicatedItems.length} hóa đơn trùng lặp với giá gấp đôi giữa hai file`
  );
 ...
 */
export const areRecordsIdentical = (
  item1: InvoiceItem,
  item2: InvoiceItem
): boolean => {
  ...
  return (
    item1.invoiceOriginal === item2.invoiceOriginal &&
 ...
 * @param file1NetPriceCol - Cột doanh số chưa thuế trong file 1
 ...
 * @param file2NetPriceCol - Cột doanh số chưa thuế trong file 2
 * @param enableNetPriceComparison - Có bật so sánh doanh số chưa thuế hay không
...
export const compareInvoiceData = (
  ...
  enableNetPriceComparison: boolean = true
): ComparisonResult => {
  // Trích xuất dữ liệu từ cả hai file
  const file1Result = extractInvoiceData(
    file1Data,
    file1StartRow,
    file1InvoiceCol,
    file1SellerCol,
  ...
  const file2Result = extractInvoiceData(
    file2Data,
    file2StartRow,
    file2InvoiceCol,
    file2SellerCol,
  ...
  console.log(
    "So sánh:",
    file1Keys.length,
    "số hóa đơn từ file 1 với",
    file2Keys.length,
    "số hóa đơn từ file 2"
  );
  ...
  const missingInFile2 = file1Keys
    .filter(
      (key) =>
        !file2Keys.includes(key) &&
        isNumber(file1Invoices[key][0].invoiceOriginal)
    )
  ...
  const missingInFile1 = file2Keys
    .filter(
      (key) =>
        !file1Keys.includes(key) &&
        isNumber(file2Invoices[key][0].invoiceOriginal)
    )
  ...
  file1Keys.forEach((key) => {
    if (file2Keys.includes(key) && isNumber(key)) {
      ...
      if (hasTaxCodeMismatch) {
        console.log(
          `=== Phát hiện hóa đơn #${key} có mã số thuế không khớp ===`
        );
        file1Items.forEach((item) => {
          console.log(
            `File1 [${item.position}]: "${item.taxCode}" → "${normalizeTaxCode(
              item.taxCode
            )}"`
          );
        });
        file2Items.forEach((item) => {
          console.log(
            `File2 [${item.position}]: "${item.taxCode}" → "${normalizeTaxCode(
              item.taxCode
            )}"`
          );
        });
  ...
  // Chỉ thực hiện so sánh netPrice khi tùy chọn được bật
  const crossFileDuplicates = enableNetPriceComparison
    ? findCrossFileDuplicates(file1AllInvoices, file2AllInvoices)
    : [];
...
export const validateInput = (
  ...
  if (
    invoiceCol >= firstRow.length ||
    sellerCol >= firstRow.length ||
    taxCodeCol >= firstRow.length
  ) {
